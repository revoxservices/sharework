                            ...
                            <div class="nk-block-between-md g-4">
                                ...
                                <div class="nk-block-head-content">
                                    <ul class="nk-block-tools gx-3">
                                        <li><a href="{{ route('customers.teams.create', array($project->token)) }}" class="btn btn-primary">Invitar nuevo miembro</a></li>
                                    </ul>
                                </div>
                            </div>
                        </div><!-- .nk-block-head -->
                                        ...
                                        @foreach ($teams as $team)
                                            <tr class="tb-member-item">
                                                <td class="tb-member-info">
                                                    <div class="user-card">
                                                        <div class="user-avatar bg-success">
                                                            <span>{{ substr($team->firstname, 0, 1) }}{{ substr($team->lastname, 0, 1) }}</span>
                                                        </div>
                                                        <div class="user-info">
                                                            <span class="lead-text">{{ $team->firstname }} {{ $team->lastname }}</span>
                                                            <span class="sub-text">{{ $team->email }}</span>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td class="tb-member-type tb-col-sm">
                                                    @if ($team->privilege_id == 1)
                                                        <span>Dueño</span>
                                                    @else
                                                        <span>Miembro</span>
                                                    @endif
                                                ...
                                                <td class="tb-member-role tb-col-md">
                                                    @if($team->privilege_id == 1)
                                                        <span>Admin</span>
                                                    @elseif($team->privilege_id == 2)
                                                        <span>Editor</span>
                                                    @elseif($team->privilege_id == 3)
                                                        <span>Lector</span>
                                                    @endif
                                                </td>
                                                                ...
                                                                <div class="drodown">
                                                                    ...
                                                                    <div class="dropdown-menu dropdown-menu-right">
                                                                        <ul class="link-list-opt no-bdr">
                                                                            <li><a href="{{ route('customers.teams.edit', array($team->id)) }}"><em class="icon ni ni-user"></em><span>Editar Integrante</span></a></li>
                                                                            <li class="divider"></li>
                                                                            <li><a href="{{ route('customers.teams.destroy', array($team->id,$project->token)) }}"><em class="icon ni ni-trash"></em><span>Eliminar</span></a></li>
                                                                        </ul>
                                                                    </div>
                                                                </div>
                                                            </li>
